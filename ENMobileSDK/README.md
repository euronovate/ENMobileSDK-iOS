...
Add `pod 'ENMobileSDK', '1.3.2'` to your **PodFile**
...
Just after the language, you can set the `certificateSourceType`. By default, it's set to `.autogenerated`, which means that through openSSL library and apposes the generated certificate to the signed acrofield. 
...
Alternatively you can choose to pass as parameter a `.p12` base64 string, with its password to being decrypted, and the signature is signed with this certificate.
